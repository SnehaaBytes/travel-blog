...
function AdminLayout() {
  ...
  return (
    <div className="flex h-screen bg-slate-50 font-sans overflow-hidden text-slate-800">
      ...
      {/* ======================= Mobile Header ======================= */}
      <div className="md:hidden fixed top-0 w-full z-40 bg-white/80 backdrop-blur-xl border-b border-slate-200 px-5 py-3.5 flex justify-between items-center shadow-sm">
        <div className="flex items-center gap-3">
          <div className="w-9 h-9 rounded-xl bg-gradient-to-tr from-blue-600 to-indigo-600 flex items-center justify-center text-white shadow-md shadow-blue-500/20">
            <i className="fas fa-plane-departure text-sm"></i>
          </div>
          <h2 className="text-xl font-extrabold text-slate-800 tracking-tight">
            Travel<span className="text-blue-600">Admin</span>
          </h2>
        </div>
        
        <button 
          className="w-10 h-10 rounded-xl bg-slate-100 flex items-center justify-center text-slate-600 hover:bg-slate-200 hover:text-blue-600 transition-colors active:scale-95"
      ...
      >
        ...
        <div className="hidden md:flex items-center gap-3 px-8 h-24 border-b border-slate-800/60">
        ...
        <div className="p-5 border-t border-slate-800/60 flex flex-col gap-3">
          <div className="flex items-center gap-3 px-4 py-3 rounded-2xl bg-slate-800/40 border border-slate-700/50">
            ...
            <div className="flex-1 overflow-hidden">
              {/* Dynamically display the logged-in administrator's username! */}
      ...
      <main className="flex-1 flex flex-col h-screen overflow-hidden relative bg-slate-50">
        ...
        <header className="hidden md:flex h-24 items-center justify-between px-10 bg-white/80 backdrop-blur-xl border-b border-slate-200/60 z-10 sticky top-0">
          <div>
          ...
          {/* 👉 Action Tools (Now Interactive!) */}
          <div className="flex items-center gap-3">
            {/* Search Button */}
            <button 
              onClick={() => alert("Search functionality will be implemented soon!")}
              className="w-11 h-11 rounded-full bg-white border border-slate-200 text-slate-500 hover:text-blue-600 hover:border-blue-300 hover:bg-blue-50 transition-all shadow-sm flex items-center justify-center hover:-translate-y-0.5"
              title="Search Data"
            >
            ...
            <button 
              ...
              className="w-11 h-11 rounded-full bg-white border border-slate-200 text-slate-500 hover:text-blue-600 hover:border-blue-300 hover:bg-blue-50 transition-all shadow-sm flex items-center justify-center relative hover:-translate-y-0.5"
              title="Notifications"
            >
              <i className="far fa-bell"></i>
              <span className="absolute top-2.5 right-3 w-2.5 h-2.5 rounded-full bg-rose-500 border-2 border-white"></span>
            </button>
            ...
            <button 
              onClick={handleLogout}
              className="w-11 h-11 rounded-full bg-rose-50 border border-rose-100 text-rose-500 hover:text-white hover:border-rose-500 hover:bg-rose-500 transition-all shadow-sm flex items-center justify-center hover:-translate-y-0.5"
              title="Quick Logout"
            >
